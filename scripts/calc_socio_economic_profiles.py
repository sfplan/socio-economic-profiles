...
def calc_socio_economic_profiles(attribute_df, years, geo_lookup_df, output_path='./output'):
    ...
    df_all_calcs.to_csv(
        os.path.join(download_path, 'Neighborhood' + '_' + 'profiles_by_attribute_' + label + '_{}.csv'.format(year)),
        index=False)
    ...
    df_all_calcs_tp.to_csv(
        os.path.join(download_path, 'Tract' + '_' + 'profiles_by_geo_' + label + '_{}.csv'.format(year)), index=False)